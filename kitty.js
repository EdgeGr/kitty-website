...
function spawnKitties() {
  kitties.forEach(kitty => {
    ...
    const padding = 100;
    const maxX = window.innerWidth - kitty.width - padding;
    const maxY = window.innerHeight - kitty.height - padding;

    const startX = Math.random() * maxX + padding / 2;
    const startY = Math.random() * maxY + padding / 2;
...
kitties.forEach(kitty => {
  if (kitty.complete) {
    loadedCount++;
    if (loadedCount === kitties.length) init();
  } else {
